...
class ResultsView {
  ...
  async confirmResult({pending} = {}) {
    ...
    if (selectedRow instanceof MatchRow) {
      ...
      const editor = await atom.workspace.open(selectedRow.group.result.filePath, {
        pending,
        searchAllPanes: true,
        split: reverseDirections[atom.config.get('find-and-replace-plus.projectSearchResultsPaneSplitDirection')]
      })
  ...
  toggleLeadingContextLines() {
    if (this.model.getFindOptions().leadingContextLineCount > 0) {
      ...
      const searchContextLineCountBefore = atom.config.get('find-and-replace-plus.searchContextLineCountBefore');
  ...
  toggleTrailingContextLines() {
    if (this.model.getFindOptions().trailingContextLineCount > 0) {
      ...
      const searchContextLineCountAfter = atom.config.get('find-and-replace-plus.searchContextLineCountAfter');
