...
module.exports = class ResultsView {
  constructor({ model, collapseNewResults = false }) {
    ...
    this.rowGroupLengths = this.resultRowGroups.map(
      (group) => group.rows.length
    );

    this.resultRows = [].concat(
      ...this.resultRowGroups.map((group) => group.rows)
    );
    ...
    this.subscriptions = new CompositeDisposable(
      atom.config.observe(
        "editor.fontFamily",
        this.fontFamilyChanged.bind(this)
      ),
      ...
      this.model
        .getFindOptions()
        .onDidChangeReplacePattern(() => etch.update(this)),
  ...
  async invalidateItemHeights() {
    ...
    const clientHeight =
      this.refs.listView && this.refs.listView.element.clientHeight;
  ...
  invalidateRowGroup(firstRowIndex, groupIndex) {
    ...
    this.spliceRows(
      firstRowIndex,
      this.rowGroupLengths[groupIndex],
  ...
  didAddResult({ result, filePath }) {
    ...
    this.rowGroupLengths.splice(groupIndex, 0, rowGroup.displayedRows().length);
  ...
  async confirmResult({ pending } = {}) {
    ...
    if (selectedRow instanceof MatchRow) {
      ...
      const editor = await atom.workspace.open(
        selectedRow.group.result.filePath,
        {
          pending,
          searchAllPanes: true,
          split:
  ...
  async openInNewTab() {
    if (this.selectedRowIndex !== -1) {
      ...
      if (result) {
        ...
        const exists = _.some(
          editors,
          (editor) => editor.getPath() == filepath
        );
  ...
  scrollToSelectedMatch() {
    ...
    if (this.refs.listView) {
      ...
      if (
        bottom >
        this.getScrollTop() + this.refs.listView.element.clientHeight
      ) {
