    ...
    if (result && result.matches && result.matches.length) {
      ...
      for (const m of result.matches) {
        ...
        for (let row = range.start.row; row <= range.end.row; row++) {
          ...
          const endColumn =
            row === range.end.row ? range.end.column : lineText.length;
...
module.exports = class ResultsModel {
  ...
  async search(findPattern, pathsPattern, replacePattern, options = {}) {
    ...
    this.findOptions.set(
      Object.assign({ findPattern, replacePattern, pathsPattern }, options)
    );
    ...
    const leadingContextLineCount = atom.config.get(
      "find-and-replace-plus.searchContextLineCountBefore"
    );
    const trailingContextLineCount = atom.config.get(
      "find-and-replace-plus.searchContextLineCountAfter"
    );

    const startTime = Date.now();
    const useRipgrep = atom.config.get("find-and-replace-plus.useRipgrep");
    const enablePCRE2 = atom.config.get("find-and-replace-plus.enablePCRE2");
    ...
    this.inProgressSearchPromise = atom.workspace.scan(
      ...
      (result, error) => {
        if (result) {
          this.setResult(result.filePath, Result.create(result), true);
        } else {
  ...
  setResult(filePath, result, fromScan = false) {
    if (result == null) {
      return this.removeResult(filePath);
    }
    if (!this.results[filePath]) {
      return this.addResult(filePath, result, fromScan);
    }

    this.matchCount +=
      result.matches.length - this.results[filePath].matches.length;
  ...
  shouldAddResult(filePath) {
    ...
    const searchPaths = this.pathsArrayFromPathsPattern(
      this.findOptions.pathsPattern
    );
    return atom.workspace.filePathMatchesPatterns(filePath, searchPaths);
  }

  addResult(filePath, result, fromScan = false) {
    // Skip pattern filtering for results from the initial scan, since they're
    // already filtered by ripgrep/scandal. The shouldAddResult check is only
    // needed for results from onContentsModified (when files are edited during
    // an active search).
    if (!fromScan && !this.shouldAddResult(filePath)) return;
