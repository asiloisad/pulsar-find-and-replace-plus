...
module.exports = {
  activate(param) {
    ...
    if (
      atom.config.get("search-panel.openProjectFindResultsInRightPane")
    ) {
      atom.config.set(
    ...
    atom.config.unset(
      "search-panel.openProjectFindResultsInRightPane"
    );
    ...
    this.subscriptions.add(
      atom.commands.add("atom-workspace", "find-and-replace:clear", () => {
        this.findView?.clear();
  ...
  createViews() {
    ...
    return this.toggleAutocompletions(
      atom.config.get("search-panel.autocompleteSearches")
    );
  },
