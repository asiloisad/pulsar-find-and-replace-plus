...
module.exports = {
  activate(param) {
    ...
    if (
      atom.config.get("find-and-replace-plus.openProjectFindResultsInRightPane")
    ) {
      atom.config.set(
    ...
    atom.config.unset(
      "find-and-replace-plus.openProjectFindResultsInRightPane"
    );
  ...
  toggleAutocompletions(value) {
    ...
    if (value) {
      ...
      const disposable = this.autocompleteWatchEditor?.(
        this.findView.findEditor,
        ["default"]
      );
      if (disposable != null) {
  ...
  consumeAutocompleteWatchEditor(watchEditor) {
    ...
    atom.config.observe("find-and-replace-plus.autocompleteSearches", (value) =>
  ...
  provideDirectorySearcher() {
    // Provide our custom searcher with CRLF and multiline fixes
    // The searcher's canSearchDirectory() checks if ripgrep is enabled
  ...
  createViews() {
    ...
    return this.toggleAutocompletions(
      atom.config.get("find-and-replace-plus.autocompleteSearches")
    );
  },
