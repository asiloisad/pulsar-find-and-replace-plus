...
module.exports = {
  activate(param) {
    ...
    if (atom.config.get('find-and-replace-plus.openProjectFindResultsInRightPane')) {
      atom.config.set('find-and-replace-plus.projectSearchResultsPaneSplitDirection', 'right');
    }
    atom.config.unset('find-and-replace-plus.openProjectFindResultsInRightPane');
  ...
  consumeAutocompleteWatchEditor(watchEditor) {
    ...
    atom.config.observe(
      'find-and-replace-plus.autocompleteSearches',
  ...
  createViews() {
    ...
    return this.toggleAutocompletions(atom.config.get('find-and-replace-plus.autocompleteSearches'));
  },
