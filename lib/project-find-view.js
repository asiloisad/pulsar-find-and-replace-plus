...
module.exports = class ProjectFindView {
        ...
        $.span(
          ...
          $.span(
            ...
            $.button(
              ...
              $.svg({
                className: "icon",
                innerHTML: `<use xlink:href="#find-and-replace-icon-ripgrep" />`,
              })
            ),
            $.button(
              ...
              $.svg({
                className: "icon",
                innerHTML: `<use xlink:href="#find-and-replace-icon-pcre2" />`,
              })
            )
          )
        )
      ...
      $.section(
        ...
        $.div(
          ...
          $.div(
            ...
            $.button(
              { ref: "replaceAllButton", className: "btn disabled" },
              "Replace All"
            )
          )
        )
  ...
  handleEvents() {
    ...
    this.subscriptions.add(
      atom.commands.add(this.element, {
        ...
        "project-find:toggle-whole-word-option": () =>
          this.toggleWholeWordOption(),
        "project-find:replace-all": () => this.replaceAll(),
        "find-and-replace:clear": () => this.clear(),
      })
    );
    ...
    let afterSearch = () => {
      if (atom.config.get("find-and-replace-plus.closeFindPanelAfterSearch")) {
    ...
    const addModelHandlers = () => {
      ...
      this.modelSubscriptions.add(
        this.model.onDidClearReplacementState(updateInterfaceForResults)
      );
      this.modelSubscriptions.add(
        this.model.onDidStartSearching(updateInterfaceForSearching)
      );
      this.modelSubscriptions.add(this.model.onDidNoopSearch(afterSearch));
      this.modelSubscriptions.add(
        this.model.onDidFinishSearching(searchFinished)
      );
      this.modelSubscriptions.add(
        this.model
          .getFindOptions()
          .onDidChange(this.updateOptionViews.bind(this))
      );
      this.modelSubscriptions.add(
        this.model
          .getFindOptions()
    ...
    this.refs.regexOptionButton.addEventListener("click", () =>
      this.toggleRegexOption()
    );
    this.refs.caseOptionButton.addEventListener("click", () =>
      this.toggleCaseOption()
    );
    this.refs.wholeWordOptionButton.addEventListener("click", () =>
      this.toggleWholeWordOption()
    );
    this.refs.ripgrepOptionButton.addEventListener("click", () =>
      this.toggleRipgrepOption()
    );
    this.refs.pcre2OptionButton.addEventListener("click", () =>
      this.togglePCRE2Option()
    );
    this.refs.replaceAllButton.addEventListener("click", () =>
      this.replaceAll()
    );
  ...
  handleEventsForReplace() {
    this.editorSubscriptions.add(
      this.replaceEditor
        .getBuffer()
        .onDidChange(() => this.model.clearReplacementState()),
      this.replaceEditor.onDidStopChanging(() => {
        this.model
          .getFindOptions()
          .set({ replacePattern: this.replaceEditor.getText() });
      })
    );
    ...
    const addReplaceModelHandlers = () => {
      ...
      this.modelSubscriptions.add(
        this.model.onDidReplacePath((result) => {
          ...
          this.refs.replacementProgress.value =
            this.replacementsMade / this.model.getPathCount();
          this.refs.replacmentInfo.textContent = `Replaced ${
            this.replacementsMade
          } of ${_.pluralize(this.model.getPathCount(), "file")}`;
        })
      );

      this.modelSubscriptions.add(
        this.model.onDidFinishReplacing((result) =>
          this.onFinishedReplacing(result)
        )
      );
    };
  ...
  replaceAll() {
    ...
    return this.showResultPane().then(() => {
      ...
      const message = `This will replace '${findPattern}' with '${replacePattern}' ${_.pluralize(
        this.model.matchCount,
        "time"
      )} in ${_.pluralize(this.model.pathCount, "file")}`;
  ...
  showResultPane() {
    ...
    let openDirection = atom.config.get(
      "find-and-replace-plus.projectSearchResultsPaneSplitDirection"
    );
  ...
  updateEngineOptionButtons() {
    this.setOptionButtonState(
      ...
      atom.config.get("find-and-replace-plus.useRipgrep")
    );
    this.setOptionButtonState(
      this.refs.pcre2OptionButton,
      atom.config.get("find-and-replace-plus.enablePCRE2")
    );
  }
  ...
  toggleRipgrepOption() {
    const newValue = !atom.config.get("find-and-replace-plus.useRipgrep");
    atom.config.set("find-and-replace-plus.useRipgrep", newValue);
    this.updateEngineOptionButtons();
  }

  togglePCRE2Option() {
    const newValue = !atom.config.get("find-and-replace-plus.enablePCRE2");
    atom.config.set("find-and-replace-plus.enablePCRE2", newValue);
    this.updateEngineOptionButtons();
  }
