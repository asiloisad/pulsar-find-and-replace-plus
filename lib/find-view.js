...
class FindView {
  ...
  confirm() {
    this.findNext({focusEditorAfter: atom.config.get('find-and-replace-plus.focusEditorAfterSearch')});
  }

  showPrevious() {
    this.findPrevious({focusEditorAfter: atom.config.get('find-and-replace-plus.focusEditorAfterSearch')});
  }

  liveSearch() {
    let findPattern = this.findEditor.getText();
    if (findPattern.length === 0 || (findPattern.length >= atom.config.get('find-and-replace-plus.liveSearchMinimumCharacters') && !this.model.patternMatchesEmptyString(findPattern))) {
  ...
  markersUpdated(markers) {
    ...
    if (this.model.getFindOptions().findPattern) {
      ...
      if (this.findEditor.getElement().hasFocus() && results > 0 && atom.config.get('find-and-replace-plus.scrollToResultOnLiveSearch')) {
