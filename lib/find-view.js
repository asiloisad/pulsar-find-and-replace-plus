...
module.exports = class FindView {
  constructor(
    model,
    { findBuffer, replaceBuffer, findHistoryCycler, replaceHistoryCycler } = {}
  ) {
    this.model = model;
    ...
    return $.div(
        ...
        $.span(
          ...
          $.span(
            ...
            $.button(
              ...
              $.svg({
                className: "icon",
                innerHTML: '<use href="#find-and-replace-icon-case" />',
              })
            ),
            ...
            $.button(
              ...
              $.svg({
                className: "icon",
                innerHTML: '<use href="#find-and-replace-icon-word" />',
              })
            )
          )
        )
      ...
      $.section(
        ...
        $.div(
          ...
          $.div(
            ...
            $.button(
              { ref: "replaceNextButton", className: "btn btn-next" },
              "Replace"
            )
          ),
          ...
          $.div(
            ...
            $.button(
              { ref: "replaceAllButton", className: "btn btn-all" },
              "Replace All"
            )
          )
        )
      ...
      $.svg({
        ...
        innerHTML: `
          ...
          <symbol id="find-and-replace-icon-ripgrep" viewBox="0 0 20 16" stroke="none" fill-rule="evenodd">
          ...
          <symbol id="find-and-replace-icon-pcre2" viewBox="0 0 20 16" stroke="none" fill-rule="evenodd">
  ...
  handleEvents() {
    ...
    this.subscriptions.add(
      atom.commands.add("atom-workspace", {
        ...
        "find-and-replace:find-all": () =>
          this.findAll({ focusEditorAfter: true }),
        "find-and-replace:find-next-selected": this.findNextSelected.bind(this),
        "find-and-replace:find-previous-selected": this.findPreviousSelected.bind(
          this
        ),
        "find-and-replace:use-selection-as-find-pattern": this.setSelectionAsFindPattern.bind(
          this
        ),
        "find-and-replace:use-selection-as-replace-pattern": this.setSelectionAsReplacePattern.bind(
          this
        ),
      })
    );
    ...
    this.subscriptions.add(
      atom.commands.add(this.element, {
        ...
        "find-and-replace:toggle-regex-option": this.toggleRegexOption.bind(
          this
        ),
        "find-and-replace:toggle-case-option": this.toggleCaseOption.bind(this),
        "find-and-replace:toggle-selection-option": this.toggleSelectionOption.bind(
          this
        ),
        "find-and-replace:toggle-whole-word-option": this.toggleWholeWordOption.bind(
          this
        ),
        "find-and-replace:clear": this.clear.bind(this),
      })
    );

    this.subscriptions.add(
      this.model.onDidUpdate(this.markersUpdated.bind(this))
    );
    ...
    this.subscriptions.add(
      this.model
        .getFindOptions()
        .onDidChangeUseRegex(this.updateSyntaxHighlighting.bind(this))
    );
    ...
    this.refs.regexOptionButton.addEventListener(
      "click",
      this.toggleRegexOption.bind(this)
    );
    this.refs.caseOptionButton.addEventListener(
      "click",
      this.toggleCaseOption.bind(this)
    );
  ...
  confirm() {
    this.findNext({
      focusEditorAfter: atom.config.get(
        "find-and-replace-plus.focusEditorAfterSearch"
      ),
    });
  }
  ...
  liveSearch() {
    ...
    if (
      ...
      (findPattern.length >=
        atom.config.get("find-and-replace-plus.liveSearchMinimumCharacters") &&
  ...
  replaceCurrent() {
    ...
    if (this.markers && this.markers.length > 0) {
      ...
      if (!currentMarker) {
        let position = this.firstMarkerIndexStartingFromCursor();
        if (position) {
  ...
  markersUpdated(markers) {
    ...
    if (this.model.getFindOptions().findPattern) {
      ...
      let resultsStr = results
        ? `${results} ${pluralize(results, "result")}`
        : "No results";
      this.element.classList.remove("has-results", "has-no-results");
      this.element.classList.add(results ? "has-results" : "has-no-results");
      this.setInfoMessage(
        `${resultsStr} found for '${this.model.getFindOptions().findPattern}'`
      );
      if (
        this.findEditor.getElement().hasFocus() &&
        results > 0 &&
        atom.config.get("find-and-replace-plus.scrollToResultOnLiveSearch")
      ) {
  ...
  updateResultCounter() {
    ...
    const index =
      this.markers && this.markers.indexOf(this.model.currentResultMarker);
  ...
  selectAllMarkers() {
    ...
    let ranges = Array.from(this.markers).map((marker) =>
    ...
    editor.scrollToBufferPosition(scrollMarker.getStartBufferPosition(), {
      center: true,
    });
  }
  ...
  updateSyntaxHighlighting() {
    if (this.model.getFindOptions().useRegex) {
      this.findEditor.setGrammar(
        atom.grammars.grammarForScopeName("source.js.regexp")
      );
  ...
  updateOptionButtons() {
    this.setOptionButtonState(
      this.refs.regexOptionButton,
      this.model.getFindOptions().useRegex
    );
    this.setOptionButtonState(
  ...
  toggleSelectionOption() {
    ...
    if (
      editor &&
      editor.getSelectedBufferRanges().every((range) => range.isEmpty())
    ) {
      this.selectFirstMarkerAfterCursor();
    }
  }
  ...
  updateReplaceEnablement() {
    ...
    if (
      canReplace &&
      !this.refs.replaceAllButton.classList.contains("disabled")
    )
      return;

    if (this.replaceTooltipSubscriptions)
      this.replaceTooltipSubscriptions.dispose();
  ...
  showWrapIcon(icon) {
    if (!atom.config.get("find-and-replace-plus.showSearchWrapIcon")) return;
    let editor = this.model.getEditor();
    if (!editor) return;
    ...
    this.wrapTimeout = setTimeout(
      () => this.wrapIcon.classList.remove("visible"),
      500
    );
  }
};
